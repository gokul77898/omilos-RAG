...
|-------|------------|---------|
| **Frontend** | React 19 + Vite 7 + TailwindCSS 4 | Chat UI, Document Viewer, KG Visualizer, Analytics |
| **Backend** | FastAPI + SSE Streaming | REST API, Agentic Chat, Function Calling, Citations |
| **ML Models** | bge-m3, bge-reranker-v2-m3 | Embedding (1024-dim), Cross-encoder Reranking |
| **LLM Providers** | Gemini API, Ollama | 2.5 Flash / 3.x Thinking, Local gemma3/qwen3.5 |
...
Most RAG systems follow a simple pipeline: split text → embed → retrieve → generate. OmilosRAG goes further at every stage:

| Aspect | Traditional RAG | OmilosRAG |
|---|---|---|
...
OmilosRAG supports two document parsers, switchable via `OMILOSRAG_DOCUMENT_PARSER` env config:
...
OMILOSRAG_DOCUMENT_PARSER=marker   # or "docling" (default)
```
...
> **Tip**: Gemma 4 models require **Ollama v0.20.0+**. OmilosRAG auto-detects native tool calling support — models that support it use Ollama's native tools API (more reliable), others fall back to prompt-based tool calling automatically.
...
OmilosRAG supports multiple graph storage backends via LightRAG:

| Storage | Use Case | Setup |
|---------|----------|-------|
| **NetworkXStorage** (default) | Small-medium datasets (<1M entities) | No setup — file-based, works out of the box |
| **Neo4JStorage** | Large-scale production (millions of entities) | Requires Neo4j server |
| **PGGraphStorage** | Use existing PostgreSQL | Requires PostgreSQL with graph extensions |
| **MongoGraphStorage** | MongoDB users | Requires MongoDB server |

**Neo4j Setup** (for scalable production graphs):
...
OMILOSRAG_KG_GRAPH_STORAGE=Neo4JStorage
```

**Switch back to file-based** (no external database):
```bash
OMILOSRAG_KG_GRAPH_STORAGE=NetworkXStorage
```

> **Note**: The animated graph visualization in the UI works with any storage backend. Neo4j is only needed for datasets with millions of entities or when you need advanced graph queries.
...
OmilosRAG was evaluated using two complementary methods: **16 hand-crafted tests** (rule-based metrics) and **30 RAGAS synthetic tests** (LLM-as-judge). Test corpus: TechVina Annual Report 2025 (Vietnamese, 26 chunks) + DeepSeek-V3.2 Technical Paper (English, 57 chunks).
...
```bash
git clone https://github.com/LeDat98/OmilosRAG.git
cd OmilosRAG
cp .env.example .env
...
| Variable | Default | Description |
|---|---|---|
| `OMILOSRAG_EMBEDDING_MODEL` | `BAAI/bge-m3` | Embedding model (1024-dim) |
| `OMILOSRAG_RERANKER_MODEL` | `BAAI/bge-reranker-v2-m3` | Cross-encoder reranker |
| `OMILOSRAG_VECTOR_PREFETCH` | `20` | Candidates before reranking |
| `OMILOSRAG_RERANKER_TOP_K` | `8` | Final results after reranking |
| `OMILOSRAG_ENABLE_KG` | `true` | Enable knowledge graph extraction |
| `OMILOSRAG_DOCUMENT_PARSER` | `docling` | Document parser: `docling` (default) or `marker` (lighter, better math) |
| `OMILOSRAG_MARKER_USE_LLM` | `false` | Enable LLM-enhanced mode for Marker (better tables, equations) |
| `OMILOSRAG_ENABLE_IMAGE_EXTRACTION` | `true` | Extract images from documents |
| `OMILOSRAG_ENABLE_IMAGE_CAPTIONING` | `true` | LLM-caption images for search |
| `OMILOSRAG_KG_LANGUAGE` | `Vietnamese` | KG extraction language |
...
- [x] **Marker PDF Parser** — Add [Marker](https://github.com/datalab-to/marker) as an alternative document parser with superior math/formula extraction (LaTeX via Surya), lighter GPU footprint (~2-4GB VRAM vs Docling's ~18-20GB), and switchable via `OMILOSRAG_DOCUMENT_PARSER=marker` env config

---

## MCP Server

OmilosRAG includes a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that exposes its core functionalities:
...
You can directly add the OmilosRAG MCP server to Cursor or Claude as an SSE connection using the URL:
