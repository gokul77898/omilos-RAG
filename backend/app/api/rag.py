...
async def query_documents(
    ...
    if isinstance(rag_service, OmilosRAGService) and request.mode != "vector_only":
...
async def process_document(
    ...
    if document.status in (DocumentStatus.PROCESSING, DocumentStatus.PARSING, DocumentStatus.INDEXING):
        ...
        timeout = settings.OMILOSRAG_PROCESSING_TIMEOUT_MINUTES
...
async def reindex_document(
    ...
    """Re-process an existing document through the OmilosRAG pipeline."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    try:
        ...
        return DocumentProcessResponse(
            ...
            message=f"Re-indexed with OmilosRAG: {chunk_count} chunks created"
        )
    except Exception as e:
        raise HTTPException(
...
async def reindex_workspace(
    ...
    and re-processes every document through the OmilosRAG pipeline.
...
async def get_workspace_rag_stats(
    ...
    # Count OmilosRAG documents (parser_version = 'docling')
    omilosrag_result = await db.execute(
        select(func.count(Document.id)).where(
            Document.workspace_id == workspace_id,
            Document.parser_version == "docling"
        )
    )
    omilosrag_documents = omilosrag_result.scalar() or 0
    ...
    return ProjectRAGStatsResponse(
        ...
        omilosrag_documents=omilosrag_documents,
    )
...
async def _get_kg_service(workspace_id: int):
    """Get KnowledgeGraphService for a knowledge base (if OmilosRAG is active)."""
...
async def get_workspace_analytics(
    ...
    # KG analytics (optional — only if OmilosRAG active)
    kg_analytics = None
    if omilosrag_documents > 0:
        try:
            kg = await _get_kg_service(workspace_id)
...
# ---------------------------------------------------------------------------
# Chat endpoint — LLM-powered document Q&A via OmilosRAG
# ---------------------------------------------------------------------------
...
async def chat_with_documents(
    ...
    """Chat with documents using OmilosRAG retrieval + LLM answer generation."""
    kb = await verify_workspace_access(workspace_id, db)

    rag_service = get_rag_service(db, workspace_id)

    # -- 1. Retrieve relevant chunks via OmilosRAG --
    chunks = []
    citations = []
    kg_summary = ""
