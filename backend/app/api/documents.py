...
async def get_document_markdown(
    ...
    """Get the full structured markdown content of a document (OmilosRAG parsed)."""
    result = await db.execute(select(Document).where(Document.id == document_id))
    document = result.scalar_one_or_none()
    ...
    if not document.markdown_content:
        raise HTTPException(
            ...
            detail="No markdown content available. Document may not have been processed with OmilosRAG."
        )
