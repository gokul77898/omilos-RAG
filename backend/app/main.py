"""
OmilosRAG — standalone Knowledge Base + RAG application.
"""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(f"✓ Database connected: {settings.DATABASE_URL.split('@')[-1] if '@' in settings.DATABASE_URL else 'OK'}")
    except Exception as e:
        logger.error(f"✗ Database connection failed: {e}")
        logger.error("Please check DATABASE_URL in .env and ensure PostgreSQL is running")
...
app = FastAPI(
    ...
    description="OmilosRAG — Knowledge Base with semantic search, knowledge graph, and LLM chat",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
# Static files — document images extracted by OmilosRAG (Docling)
