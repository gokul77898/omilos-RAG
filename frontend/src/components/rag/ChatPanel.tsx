...
export const ChatPanel = memo(function ChatPanel({
  ...
  const [debugMode, setDebugMode] = useState(() =>
    localStorage.getItem("omilosrag-debug-mode") === "true",
  );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && e.key === "D") {
        ...
        setDebugMode((prev) => {
          const next = !prev;
          localStorage.setItem("omilosrag-debug-mode", String(next));
    ...
    <div className="h-full flex flex-col border-r min-h-0">
      ...
      {messages.length === 0 ? (
        ...
        <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto px-3 py-3 space-y-3 relative">
          ...
          {/* Debug Panel - shows retrieval stats */}
          {debugMode && messages.length > 0 && (
            <div className="mt-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
              <div className="text-[10px] font-mono text-amber-600 dark:text-amber-400 font-semibold mb-2">
                DEBUG PANEL
              </div>
              <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
                {messages.map((msg) => {
                  const lastMsg = msg;
                  const sourceCount = lastMsg.sources?.length || 0;
                  const entityCount = lastMsg.relatedEntities?.length || 0;
                  const imageCount = lastMsg.imageRefs?.length || 0;
                  return (
                    <div key={msg.id} className="col-span-2 grid grid-cols-4 gap-1">
                      <div className="flex items-center gap-1">
                        <span className="text-muted-foreground">Chunks:</span>
                        <span className="text-amber-600">{sourceCount}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-muted-foreground">KG Hits:</span>
                        <span className="text-amber-600">{entityCount}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-muted-foreground">Images:</span>
                        <span className="text-amber-600">{imageCount}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-muted-foreground">LLM:</span>
                        <span className="text-amber-600">gemma4:e4b</span>
                      </div>
                    </div>
                  );
                })}
